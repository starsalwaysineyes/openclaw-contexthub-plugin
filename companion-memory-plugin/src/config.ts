import type { MemoryPluginConfig, RecallLayer } from "./types.js";

function parseCsv(value: unknown, fallback: string[]): string[] {
...
function parseArray(raw: unknown, fallback: string[]): string[] {
...
function parseLayers(raw: unknown, fallback: RecallLayer[]): RecallLayer[] {
  return parseArray(raw, fallback).map((item) => item.toLowerCase() as RecallLayer).filter((item) => ["l0", "l1", "l2"].includes(item));
}

export function resolveConfig(raw: Record<string, unknown> | undefined): MemoryPluginConfig {
  ...
  const preAnswer = (recall.preAnswer ?? {}) as Record<string, unknown>;
  const read = (cfg.read ?? {}) as Record<string, unknown>;
