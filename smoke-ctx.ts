...
type RegisteredCommand = {
  name: string;
  handler: (ctx: CommandContext) => Promise<CommandReply | void> | CommandReply | void;
};
...
type MemorySearchPayload = QueryPayload & {
  memorySearch?: {
    results?: Array<{
      path?: string;
      recordId?: string;
...
function firstLines(text: string | undefined, count: number): string[] {
  if (!text) return [];
  return text.split("\n").slice(0, count);
}

function expectText(reply: CommandReply | void, label: string): string {
  const text = reply?.text;
  if (!text) throw new Error(`${label} returned no text`);
  if (reply?.isError) throw new Error(`${label} returned error: ${text}`);
  return text;
}
...
function buildCommandContext(args: string | undefined, commandBody: string): CommandContext {
  return {
    senderId: "ctx-smoke",
    channel: "cli-smoke",
    channelId: "cli-smoke",
...
async function main() {
  ...
  const helpText = expectText(
    await ctxCommand.handler(buildCommandContext(undefined, "/ctx")),
    "ctx help",
  );
  const recallText = expectText(
    await recallCommand.handler(buildCommandContext(undefined, "/contexthub-recall")),
    "contexthub-recall",
  );

  const sharedQueryArgs = `${query} :: ${partitionsCsv} :: ${layersCsv} :: :: ${limit} :: ${rerank} --json`;
  const ctxQueryText = expectText(
    await ctxCommand.handler(buildCommandContext(`q ${sharedQueryArgs}`, `/ctx q ${sharedQueryArgs}`)),
    "ctx query",
  );
  const ctxMemorySearchText = expectText(
    await ctxCommand.handler(buildCommandContext(`ms ${sharedQueryArgs}`, `/ctx ms ${sharedQueryArgs}`)),
    "ctx memory search",
  );
  const directQueryText = expectText(
    await directQueryCommand.handler(buildCommandContext(sharedQueryArgs, `/contexthub-query ${sharedQueryArgs}`)),
    "contexthub-query",
  );

  const ctxQueryPayload = parseJson<QueryPayload>(ctxQueryText, "ctx query");
  const ctxMemorySearchPayload = parseJson<MemorySearchPayload>(ctxMemorySearchText, "ctx memory search");
  const directQueryPayload = parseJson<QueryPayload>(directQueryText, "contexthub-query");
  ...
  if (!recordId) throw new Error("ctx query returned no recordId in the first hit");

  const ctxReadArgs = `r ${recordId} :: 1 :: ${readLines} --json`;
  const ctxReadText = expectText(
    await ctxCommand.handler(buildCommandContext(ctxReadArgs, `/ctx ${ctxReadArgs}`)),
    "ctx read",
  );
  const ctxReadPayload = parseJson<ReadPayload>(ctxReadText, "ctx read");

  const ctxMemoryGetArgs = `mg record:${recordId} :: 1 :: ${readLines} --json`;
  const ctxMemoryGetText = expectText(
    await ctxCommand.handler(buildCommandContext(ctxMemoryGetArgs, `/ctx ${ctxMemoryGetArgs}`)),
    "ctx memory get",
  );
  const ctxMemoryGetPayload = parseJson<MemoryGetPayload>(ctxMemoryGetText, "ctx memory get");
  ...
  const summary = {
    ...
    recallConfig: parseJson<Record<string, unknown>>(recallText, "contexthub-recall"),
    ctxQuery: {
